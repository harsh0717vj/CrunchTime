...
import { getAuth, onAuthStateChanged, signInWithRedirect, getRedirectResult, signOut, GoogleAuthProvider } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js";
...
const ACTIVE_GEMINI_KEY = localStorage.getItem("crunchtime_gemini_key") || "";
...
// Capture incoming redirection payload from Google chooser page
...
const DEFAULT_TASKS = [
  {
    ...
    deadline: getFutureTimeISO(2.5), // 2.5 hours from now
    priority: "high",
    completed: false,
  ...
  },
  {
    ...
    deadline: getFutureTimeISO(27), // 27 hours from now
    priority: "medium",
    completed: false,
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Key Activation Gateway Handler
  const keyGatewayOverlay = document.getElementById("key-gateway-overlay");
  const gatewayInputKey = document.getElementById("gateway-input-key");
  ...
  if (keyGatewayOverlay && gatewayInputKey && btnActivateDashboard) {
    const handleKeySubmission = () => {
      const value = gatewayInputKey.value.trim();
      
      // Perform validation: ensure not blank and length > 10 characters (no prefix restrictions)
      if (!value || value.length <= 10) {
        if (gatewayError) {
          gatewayError.textContent = "Invalid API Key format. Key must be longer than 10 characters.";
          gatewayError.classList.remove("hidden");
        }
        gatewayInputKey.style.outline = "2px solid var(--color-red)";
        gatewayInputKey.style.borderColor = "var(--color-red)";
        gatewayInputKey.focus();
      } else {
        if (gatewayError) gatewayError.classList.add("hidden");
        gatewayInputKey.style.outline = "";
        gatewayInputKey.style.borderColor = "";
        
        localStorage.setItem("crunchtime_gemini_key", value);
        keyGatewayOverlay.classList.add("hidden");
        
        // Trigger cascading global interface refresh
        renderTasks();
        updateStats();
        updateProductivityReport();

        // Seamlessly execute the original AI action that was pending
        if (pendingAIAction) {
          pendingAIAction();
    ...
    btnActivateDashboard.addEventListener("click", handleKeySubmission);
    gatewayInputKey.addEventListener("keydown", (e) => {
      if (e.key === "Enter") handleKeySubmission();
    });
  }
  ...
  tasksContainer.addEventListener("click", (e) => {
    ...
    if (btn.classList.contains("btn-complete")) {
      ...
      const hasKey = localStorage.getItem("crunchtime_gemini_key");
      if (!hasKey || hasKey.trim() === "") {
        pendingAIAction = () => {
          requestAIHelpForTask(task);
        };
        if (keyGatewayOverlay) keyGatewayOverlay.classList.remove("hidden");
        return;
      }
      requestAIHelpForTask(task);
    }
  ...
  function appendChatMessage(type, text) {
    ...
    if (isIncoming) {
      // CrunchTime brand logo in red
      iconHTML = `
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 32px; height: 32px; display: block;">
          <path d="M12 2C6.47715 2 2 6.47715 2 12C2 13.9213 2.54411 15.7157 3.4878 17.2348L2.0543 21.2863C1.9442 21.5976 2.0232 21.9446 2.2576 22.179C2.492 22.4134 2.839 22.4924 3.1503 22.3823L7.2018 20.9488C8.7209 21.8925 10.5153 22.4366 12 22.4366C17.52 22.4366 22 17.9595 22 12.4366C22 6.91379 17.52 2 12 2Z" fill="#e63946" />
          <path d="M7 10H17M7 14H14" stroke="#0a0a0a" stroke-width="2" stroke-linecap="round" />
        </svg>
      `;
    } else {
      // User message - Dynamic Firebase Profile Photo
      const user = auth.currentUser;
      if (user && user.photoURL) {
        iconHTML = `
          <img src="${user.photoURL}" alt="User Avatar" class="chat-user-avatar" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover; display: block;" />
        `;
      } else {
        iconHTML = `
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style="width: 32px; height: 32px; display: block; color: var(--text-secondary);">
    ...
    messageDiv.innerHTML = `
      <div class="msg-avatar" style="display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 50%; overflow: hidden; flex-shrink: 0;">
        ${iconHTML}
      </div>
  ...
  function simulateAIResponse(userQuery, associatedTask = null) {
    ...
    fetchGeminiAIHelp(userQuery, associatedTask).then(aiText => {
      ...
      hideTypingIndicator();
      appendChatMessage("incoming", "Sorry, I had a small hiccup. Please try again!");
      scrollToBottom();
    });
  }
  ...
  function updatePriorityBreakdown() {
    ...
    priorities.forEach(p => {
      const total = tasks.filter(p_item => p_item.priority === p).length;
      const completed = tasks.filter(p_item => p_item.priority === p && p_item.completed).length;
...
// ==========================================
// 4. REAL FIREBASE GOOGLE REDIRECT AUTH
// ==========================================
...
async function signInWithGoogle() {
  try {
    ...
    console.error("Sign in redirection error:", error);
  }
}
...
`;
  ...
  try {
    ...
    // Direct HTTP request utilizing the localStorage token lookup
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent`, {
      method: "POST",
      headers: { 
        "Content-Type": "application/json",
        "x-goog-api-key": localStorage.getItem("crunchtime_gemini_key") || ACTIVE_GEMINI_KEY
      },
      body: JSON.stringify({
